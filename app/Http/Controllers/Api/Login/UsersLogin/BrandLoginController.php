...
class BrandLoginController extends AuthController
{
    /**
    ...
    public function login()
    {
        ...
        //if not, then we need to filter out the brands from other domains
        $brands = $this->soDomain ? auth()->user()->brands : auth()->user()->brands->filter(function ($value, $key) {
            return in_array($this->domain->client_id, $value->clients->pluck('id')->toArray());
        });
        //end of filtering out brands from other domains
        
        if ($brands->count() > 0) {
            //logging in an default brand...
            $this->usersAccountService->setLoggedInAccount(auth()->user(), AccountType::brand(), $brands->first()->id);
        }
           //... and check if there is only one brand account matching the criteria
        if ($brands->count() == 1) {
        ...
        if ($brands->count() > 1) {
            //there is more than one brand assigned to that client, let's create a list of clients
            $accounts[AccountType::brand()] = $brands;
